...
function getArtistByIndex(array, index) {
  const artistLookup = `the artist at index ${index} is ${array[index].name}`;
  return artistLookup;
}
...
function addArtist(array, insName, insYears, insGenre, insNationality, insBio) {
  const addArtistArray = [...array];
  addArtistArray.push({"name": insName, "years": insYears, "genre": insGenre, "nationality": insNationality, "bio": insBio});
...
function artistInfo(array, insName){
  /*Your Code Here*/
  for(let i = 0; i < array.length; i++){
    if(array[i].name === insName) {
      const ArtistBio = array[i].bio;
      return ArtistBio; 
    } 
  }
}
...
function artistByCountry(array ,insNationality){
  /*Your Code Here*/
  const Namelist = [];
  for(let i =0; i < array.length; i++){
    if (array[i].nationality === insNationality){
    Namelist.push(array[i].name);
    } 
  }return Namelist;
}
